...
class Test_BmiCalculatorMain:
    ...
    def test_normal_weight(self):
        ...
        assert interpret_bmi(bmi) == f"Your BMI is {bmi}, you have a normal weight."
    
    @pytest.mark.regression
    def test_overweight(self):
        bmi = calculate_bmi(1.75, 80)
        assert interpret_bmi(bmi) == f"Your BMI is {bmi}, you are overweight."
    
    @pytest.mark.regression
    def test_obesity_classI(self):
        bmi = calculate_bmi(1.75, 90)
        assert interpret_bmi(bmi) == f"Your BMI is {bmi}, you are obese (Class I)."
        
    @pytest.mark.regression
    def test_obesity_classII(self):
        bmi = calculate_bmi(1.75, 100)
        assert interpret_bmi(bmi) == f"Your BMI is {bmi}, you are obese (Class II)."
    
    @pytest.mark.regression
    def test_obesity_classIII(self):
        bmi = calculate_bmi(1.75, 110)
        assert interpret_bmi(bmi) == f"Your BMI is {bmi}, you are obese (Class III)."
    
    @pytest.mark.regression
    def test_underweight(self):
        bmi = calculate_bmi(1.75, 50)
        assert interpret_bmi(bmi) == f"Your BMI is {bmi}, you are underweight."
    
    @pytest.mark.regression
    def test_invalid_input(self):
        bmi = calculate_bmi(0, 70)
        assert interpret_bmi(bmi) == "Invalid input. Height should be greater than 0."
