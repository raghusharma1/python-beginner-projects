...
class Test_BmiCalculatorReferenceChart:
    ...
    def test_reference_chart_valid_file(self, capsys):
        with open('bmi.csv', 'w', newline='') as file:
            ...
            writer.writerow(["Category", "BMI range - kg/m2"])
            writer.writerow(["Severe Thinness", "< 16"])
            writer.writerow(["Normal range", "18.5 - 25"])
        reference_chart()
        captured = capsys.readouterr()
        assert "Severe Thinness" in captured.out
        assert "Normal range" in captured.out

    @pytest.mark.invalid
    def test_reference_chart_empty_file(self, capsys):
        with open('bmi.csv', 'w'): pass
        reference_chart()
        captured = capsys.readouterr()
