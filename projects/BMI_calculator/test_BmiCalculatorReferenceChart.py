...
def test_valid_csv_file(monkeypatch, capsys):
    # Arrange: Mocking the csv file with known data
...
"""
    ...
    # Act: Running the function that uses the mocked file
    reference_chart()
    
    # Assert: Capturing printed output and comparing against expected table format
...
def test_empty_csv_file(monkeypatch, capsys):
    # Arrange: Mocking an empty csv file
    mock_data = ""
    m = mock_open(read_data=mock_data)
    monkeypatch.setattr("builtins.open", m)
    
    # Act: Running the function with the mocked empty file
    reference_chart()
    
    # Assert: Verifying that no table is printed
    captured = capsys.readouterr()
...
def test_missing_csv_file(monkeypatch):
    # Arrange: Mocking FileNotFoundError when trying to open a non-existent file
    monkeypatch.setattr("builtins.open", mock_open(side_effect=FileNotFoundError))
    
    # Act & Assert: Expecting FileNotFoundError to be raised
    with pytest.raises(FileNotFoundError):
        reference_chart()

@pytest.mark.invalid
def test_incorrect_format_csv_file(monkeypatch, capsys):
    # Arrange: Mocking a csv file with incorrect delimiter
    mock_data = "Category;Range\nUnderweight;<18.5\nNormal;18.5-24.9\nOverweight;25-29.9\nObese;30+\n"
    m = mock_open(read_data=mock_data)
    monkeypatch.setattr("builtins.open", m)
    
    # Act: Running the function that uses the incorrectly formatted file
    reference_chart()
    
    # Assert: Checking that data is not processed as expected due to incorrect delimiter
    captured = capsys.readouterr()
