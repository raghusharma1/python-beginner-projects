...
class Test_BmiCalculatorCalculateBmi:
    ...
    def test_calculate_bmi_normal(self):
        """
        Test to verify that the function correctly calculates BMI when given valid height and weight values.
        """
        height = 1.75  # in meters
        weight = 70  # in kilograms
        expected_bmi = round(weight / (height**2), 2)  # expected BMI
        assert calculate_bmi(height, weight) == expected_bmi, "The calculated BMI does not match the expected value."

    def test_calculate_bmi_zero_height(self):
        """
        Test to validate that the function correctly handles cases where the height is zero, 
        which would cause a division by zero error in the BMI calculation.
        """
        height = 0  # in meters
        weight = 70  # in kilograms
        assert calculate_bmi(height, weight) is None, "The function does not correctly handle a zero height."

    def test_calculate_bmi_negative_values(self):
        """
        Test to verify that the function correctly handles cases where the height or weight is negative, 
        which is not physically meaningful.
        """
        height = -1.75  # in meters
        weight = 70  # in kilograms
        assert calculate_bmi(height, weight) < 0, "The function does not correctly handle a negative height."

        height = 1.75  # in meters
        weight = -70  # in kilograms
        assert calculate_bmi(height, weight) < 0, "The function does not correctly handle a negative weight."

    def test_calculate_bmi_large_values(self):
        """
        Test to verify the function's behavior when given extremely large height and weight values.
        """
        height = 1e100  # in meters
        weight = 1e100  # in kilograms
        expected_bmi = round(weight / (height**2), 2)  # expected BMI
        assert calculate_bmi(height, weight) == expected_bmi, "The function does not correctly handle extremely large values."
