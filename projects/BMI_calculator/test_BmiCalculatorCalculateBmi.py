...
class Test_BmiCalculatorCalculateBmi:
    ...
    def test_calculate_bmi_negative_values(self):
        negative_height = -1.75  # meters
        negative_weight = -70  # kilograms
        assert calculate_bmi(negative_height, negative_weight) < 0

    def test_calculate_bmi_large_values(self):
        large_height = 1.75e307  # meters
        large_weight = 70e307  # kilograms
        assert calculate_bmi(large_height, large_weight) is not None
        assert isinstance(calculate_bmi(large_height, large_weight), float)
