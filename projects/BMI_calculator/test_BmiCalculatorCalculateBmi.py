...
class Test_BmiCalculatorCalculateBmi:
    ...
    # Test Scenario 1: Normal BMI Calculation
    def test_calculate_bmi_normal(self):
        """
        This test verifies that the function correctly calculates BMI when given valid height and weight values.
        """
        # Act
        bmi = calculate_bmi(1.75, 75)
        # Assert
        assert bmi == 24.49, f"Expected 24.49, but got {bmi}"

    # Test Scenario 2: Calculation with Zero Height
    def test_calculate_bmi_zero_height(self):
        """
        This test validates that the function correctly handles cases where the height is zero, which would cause a division by zero error in the BMI calculation.
        """
        # Act
        bmi = calculate_bmi(0, 75)
        # Assert
        assert bmi is None, f"Expected None, but got {bmi}"

    # Test Scenario 3: Calculation with Negative Values
    def test_calculate_bmi_negative_values(self):
        """
        This test verifies that the function correctly handles cases where the height or weight is negative, which is not physically meaningful.
        """
        # Act with negative height
        bmi = calculate_bmi(-1.75, 75)
        # Assert
        assert bmi < 0, f"Expected value less than 0, but got {bmi}"
        
        # Act with negative weight
        bmi = calculate_bmi(1.75, -75)
        # Assert
        assert bmi < 0, f"Expected value less than 0, but got {bmi}"

    # Test Scenario 4: Calculation with Extremely Large Values
    def test_calculate_bmi_large_values(self):
        """
        This test verifies the function's behavior when given extremely large height and weight values.
        """
        # Act
        bmi = calculate_bmi(1750000000, 7500000000)
        # Assert
        assert bmi is not None, f"Expected a valid BMI value, but got {bmi}"
