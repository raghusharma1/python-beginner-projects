...
Validation:
  This test confirms the function's ability to correctly update the score during the player’s batting turn when the player is not out, in line with the game's scoring rules.
...
# Assuming the function `user_turn` is defined in the module `main` within the `HandCricket` directory
# If there is an ImportError, it suggests the module is not found in the PYTHONPATH or there's an issue with the module's name or structure
# Make sure the directory structure is correctly set up as described and PYTHONPATH includes the project directory
