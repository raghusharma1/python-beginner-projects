...
# Assuming the module structure is as described and the main.py is in the HandCricket directory
...
class Test_MainToss:
    ...
    def test_user_wins_guessing_heads(self):
        # Mocking user input as '1' and random.randint to return 1 to simulate heads
        with patch('builtins.input', return_value='1'), patch('random.randint', return_value=1):
            result = toss()
            assert result == 1  # Expect user to win when guessing matches the toss

    @pytest.mark.smoke
    @pytest.mark.positive
    def test_user_wins_guessing_tails(self):
        # Mocking user input as '2' and random.randint to return 2 to simulate tails
    ...
    def test_user_loses_toss(self):
        # Mocking user input as '1' and random.randint to return 2 to simulate a loss
        with patch('builtins.input', return_value='1'), patch('random.randint', return_value=2):
            result = toss()
            assert result == 2  # Expect user to lose when guessing does not match the toss

    @pytest.mark.invalid
    @pytest.mark.error_handling
    def test_invalid_input_handling(self):
        # Mocking user input as 'three' which should raise a ValueError when trying to convert to int
