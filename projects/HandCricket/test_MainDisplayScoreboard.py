...
# Assuming the function `display_scoreboard` is located under `HandCricket/main.py`.
# As per the directory structure provided, the correct import statement should be:
from projects.HandCricket.main import display_scoreboard

class Test_MainDisplayScoreboard:
    @pytest.mark.positive
    def test_display_scoreboard_positive_scores(self):
        """Test positive scores to check the correct output and over count."""
        with patch('builtins.print') as mocked_print:
            ...
            assert mocked_print.call_args_list[2].args[0] == "Over 6:"
            assert mocked_print.call_args_list[3].args[0] == "Player 1: 50 runs"
            assert mocked_print.call_args_list[4].args[0] == "Player 2: 75 runs"

    @pytest.mark.valid
    def test_display_scoreboard_zero_scores(self):
        """Test zero scores to ensure the scoreboard manages zero correctly."""
        with patch('builtins.print') as mocked_print:
            display_scoreboard(0, 0, 0)
            mocked_print.assert_called_with("Player 2: 0 runs")
            assert mocked_print.call_args_list[0].args[0] == "\nScoreboard"
            assert mocked_print.call_args_list[1].args[0] == "=========="
            assert mocked_print.call_args_list[2].args[0] == "Over 1:"
            assert mocked_print.call_args_list[3].args[0] == "Player 1: 0 runs"
            assert mocked_print.call_args_list[4].args[0] == "Player 2: 0 runs"

    @pytest.mark.performance
    def test_display_scoreboard_high_over_number(self):
        """Test high over numbers to check boundary over counts and print formatting."""
        with patch('builtins.print') as mocked_print:
            display_scoreboard(20, 30, 99)
            mocked_print.assert_called_with("Player 2: 30 runs")
            assert mocked_print.call_args_list[0].args[0] == "\nScoreboard"
            assert mocked_print.call_args_list[1].args[0] == "=========="
            assert mocked_print.call_args_list[2].args[0] == "Over 100:"
            assert mocked_print.call_args_list[3].args[0] == "Player 1: 20 runs"
            assert mocked_print.call_args_list[4].args[0] == "Player 2: 30 runs"

    @pytest.mark.negative
    def test_display_scoreboard_negative_scores(self):
        """Test negative scores to ensure the scoreboard can handle negative values."""
        with patch('builtins.print') as mocked_print:
            display_scoreboard(-10, -5, 3)
            mocked_print.assert_called_with("Player 2: -5 runs")
            assert mocked_print.call_args_list[0].args[0] == "\nScoreboard"
            assert mocked_print.call_args_list[1].args[0] == "=========="
            assert mocked_print.call_args_list[2].args[0] == "Over 4:"
            assert mocked_print.call_args_list[3].args[0] == "Player 1: -10 runs"
            assert mocked_print.call_args_list[4].args[0] == "Player 2: -5 runs"

# Note: The main function call is not included as the script is intended to be run as a test suite.
